...
uint64_t sub_key1[16] = {0};
uint64_t calculationofkeys1(uint32_t C, uint32_t D,uint32_t L,uint32_t R,char mode);
uint64_t initial_permuatation1(uint64_t input,uint32_t L,uint32_t R,uint32_t C,uint32_t D,char mode);
uint64_t schedule_calculation1(uint64_t key,uint32_t C, uint32_t D,uint32_t L,uint32_t R,char mode);
uint64_t shiftingcidi1(uint32_t C, uint32_t D,uint32_t L,uint32_t R,char mode);
uint64_t calculationofkeys1(uint32_t C, uint32_t D,uint32_t L,uint32_t R,char mode);
uint64_t inverse_intial_permuatation1(uint32_t L,uint32_t R);
int testmain1(int);
...
uint64_t initial_permuatation1(uint64_t input,uint32_t L,uint32_t R,uint32_t C,uint32_t D,char mode)
{
    ...
    L=L1;
    R=R1;
     return schedule_calculation1(input,C,D,L,R,mode);
}
...
uint64_t schedule_calculation1(uint64_t key,uint32_t C, uint32_t D,uint32_t L,uint32_t R,char mode)
{
    ...
    C=C1;
    D=D1;
    return shiftingcidi1(C,D,L,R,mode);
...
//printf("Inside shifting -->  C:%d D:%d L:%d R:%d\n",C,D,L,R);
	 ...
	 for (i = 0; i< 16; i++)
   	{
        	...
        	for (j = 0; j < 48; j++)
        	{
            	...
            	sub_key1[i] <<= 1;
            	sub_key1[i] |= (permuted_choice_2 >> (56-PC2[j])) & LB64_MASK;
        	}
	...
	return calculationofkeys1(C, D,L,R,mode);
}
uint64_t calculationofkeys1(uint32_t C, uint32_t D,uint32_t L,uint32_t R,char mode)
{
...
static char P[] = {
    ...
    for (i = 0; i < 16; i++)
    {
        ...
        if (mode == 'd')
        {
            s_input = s_input ^ sub_key1[15-i];
        }
        else
        {
            s_input = s_input ^ sub_key1[i];
        }
	...
	return inverse_intial_permuatation1(L,R);
}
...
void top_function1(int inputdata)
{
    int i;
    ...
    for (i = 0; i < 16; i++)
     {
         ...
         R=0;
         D=0;
        result=initial_permuatation1(result,L,R,C,D,mode);
...
int testmain1(int inputdata)
{
    top_function1(inputdata);
    return 0;
}
...
int main(){
	  ...
	  while(!feof(fp))
	  {
		...
		//CALLING FUNCTION BODY EXPLICITLY
